...
- 요청할 때 마다 새로운 객체를 생성한다면, 메모리 낭비가 심하다. 이에 대한 해결 방안으로 스프링 컨테이너는 객체를 싱글톤으로 생성하고 관리한다.
    - 싱글톤 패턴이란 클래스의 인스턴스가 1개만 생성되는 것을 보장하는 디자인 패턴이다.
...
- 객체 생성을 담당하는 클래스에 `@Configuration`을 적용한다면, `@Bean`이 붙은 메서드마다 이미 스프링 빈이 존재하면 존재하는 빈을 반환하고, 스프링 빈이 없으면 생성해서 스프링 빈으로 등록하고
  반환하는 코드가 동적으로 만들어진다.
    - `@Bean`만 적용하면 스프링 빈으로 동록은 되지만, 싱글톤을 보장하지는 않는다.
...
- 인터페이스 기반으로 프록시를 생성하는 JDK 동적 프록시는 구체 클래스로 타입 캐스팅이 불가능하다.
  - 일반적으로는 구체 클래스를 주입받을 일이 거의 없지만, 필요한 경우가 있을 수 있다. 
  - 스프링 부트 2.0버전부터 CGLIB를 기본으로 사용하도록 했다.
    - CGLIB의 단점인 기본 생성자 필수, 생성자 2번 호출 문제(`실제 target 객체 생성 시 + 프록시 객체 생성할 때 부모 클래스의 생성자 호출`)는 스프링 4.0부터 해결 되었다.
