...
세션은 서버가 개별적으로 유지하기 때문에, 서버를 여러 개 두면 한 서버에서 인증을 하더라도 다른 서버에서는 인증이 되지 않는 상황이 발생할 수 있다.

이를 해결하기 위해 세션 스토리지를 따로 두어서 각 서버의 세션을 한곳에서 관리할 수 있다. 하지만 사용자가 많아질 경우 세션 스토리지에 부하가 간다.

JWT는 서버가 사용자의 정보를 토큰으로 만들어 클라이언트에게 전달하는 방식이다. 이후 요청이 들어오면 토큰이 유효한지 검증하기만 하면 된다. 

사용자 정보를 따로 저장하지 않기 때문에 서버의 확장성이 용이해진다. 하지만 토큰이 탈취될 수 있기 때문에 유효시간을 짧게 가져가고 Refresh 토큰을 사용하는 방식으로 구현할 수 있다. 
